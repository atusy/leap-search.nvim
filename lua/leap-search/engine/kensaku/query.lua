...
local function search(pat, opts_engine, opts_leap)
  ...
  if not ok and string.find(tostring(res), "Vim:E554: Syntax error in \\{...}", nil, true) then
    return require("leap-search.engine.vim.regex").search(query:gsub("\\{", "{"):gsub("\\}", "}"), opts_engine, opts_leap)
  end
  error(res)
end
